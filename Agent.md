...
- 目标：召回率：> 90 % , 精准率 > 85 %
  ...
  **精确率P：精确率关注模型判得准不准**，衡量的是在所有被模型预测为正类的结果中，真正属于正类的比例，数值越高说明模型给出的预测结果越可靠，误报越少，计算公式如3-13所示。

  ​                              ![img](./assets/clip_image002.png)             

  **召回率R：召回率关注模型找得全不全**，衡量的是在所有真实存在的正类样本中，被模型成功识别出来的比例，数值越高说明模型对目标信息的覆盖能力越强，漏检情况越少，计算公式如3-14所示。

  ​                              ![img](./assets/clip_image006.png)            

  **F1值：F1值是精确率与召回率的调和平均**，用于在两者之间取得平衡，能够避免单独优化精确率或召回率带来的偏置，更全面地反映模型在实体识别任务中的整体性能与实际应用价值，计算公式如3-15所示。
...
![img](./assets/89be37ffbfdc82a4b1ce8b7fb98315dc.png)
...
中间件实现的话：可以考虑Elasticsearch

​	实现策略：使用es官方推荐“召回+重排序”两阶段实现

​		**一阶段**，我们会通过向量检索先从知识库中选出一个比较大的候选集，比如是 topK 的 10 倍。这个阶段的目标很明确，就是“求全”。不管搜索的内容精不精准，只要语义上跟用户输入问题相关，我们就都拿出来，不遗漏任何一个潜在的结果。

​		**二阶段**，我们用调用 Elasticsearch 的 rescore 机制。也就是说，我们不再对所有文档做关键词匹配，而只是对刚刚召回的那一批候选集，再做一次 BM25 的关键词打分。这个阶段的目标是“求准”了—— 那些跟用户关键词完全匹配的结果，或者说那些“说到点子上”的结果，会被调高排名。
...
3、https://javaguide.cn/home.html
